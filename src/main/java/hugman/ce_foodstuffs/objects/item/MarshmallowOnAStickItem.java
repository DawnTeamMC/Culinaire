...
public class MarshmallowOnAStickItem extends Item {
	...
	public int getDefaultMaxBurnTime(Item item) {
		...
		else if(item == CEFItems.GOLDEN_MARSHMALLOW_ON_A_STICK) {
			return 20;
		}
		else if(item == CEFItems.BURNT_MARSHMALLOW_ON_A_STICK) {
			return 30;
		}
		else {
			return 60;
	...
	public Item getDefaultBurnItem(Item item) {
		if(item == CEFItems.UNCOOKED_MARSHMALLOW_ON_A_STICK) {
			...
			return CEFItems.GOLDEN_MARSHMALLOW_ON_A_STICK;
		}
		else if(item == CEFItems.GOLDEN_MARSHMALLOW_ON_A_STICK) {
			return CEFItems.BURNT_MARSHMALLOW_ON_A_STICK;
		}
		else if(item == CEFItems.BURNT_MARSHMALLOW_ON_A_STICK) {
			return Items.STICK;
		}
