...
public class CulinaireLootTables {
    ...
    public static void addToVanillaTables() {
        LootTableEvents.MODIFY.register((resourceManager, builder, id, wrapperLookup, source) -> {
            // Lettuce Seeds
            if (LootTables.SIMPLE_DUNGEON_CHEST.equals(id) || LootTables.ABANDONED_MINESHAFT_CHEST.equals(id) || LootTables.WOODLAND_MANSION_CHEST.equals(id)) {
                LootPool.Builder pool = LootPool.builder()
                        ...
                        .with(ItemEntry.builder(VegetableContent.LETTUCE_SEEDS).weight(10).apply(SetCountLootFunction.builder(UniformLootNumberProvider.create(2.0F, 4.0F))));
                builder.pool(pool);
            }
            ...
            if (LootTables.PILLAGER_OUTPOST_CHEST.equals(id)) {
                LootPool.Builder pool = LootPool.builder()
                        ...
                        .with(ItemEntry.builder(VegetableContent.TOMATO).weight(5).apply(SetCountLootFunction.builder(UniformLootNumberProvider.create(2.0F, 5.0F))));
                builder.pool(pool);
            }
            if (LootTables.SHIPWRECK_SUPPLY_CHEST.equals(id)) {
                LootPool.Builder pool = LootPool.builder()
                        .rolls(UniformLootNumberProvider.create(1.0F, 3.0F))
                        .conditionally(RandomChanceLootCondition.builder(0.45F))
                        .with(ItemEntry.builder(VegetableContent.TOMATO).weight(7).apply(SetCountLootFunction.builder(UniformLootNumberProvider.create(2.0F, 6.0F))));
                builder.pool(pool);
            }
            if (LootTables.VILLAGE_TAIGA_HOUSE_CHEST.equals(id) || LootTables.VILLAGE_SNOWY_HOUSE_CHEST.equals(id) || LootTables.VILLAGE_PLAINS_CHEST.equals(id)) {
                LootPool.Builder pool = LootPool.builder()
                        ...
                        .with(ItemEntry.builder(VegetableContent.TOMATO).weight(10).apply(SetCountLootFunction.builder(UniformLootNumberProvider.create(1.0F, 4.0F))));
                builder.pool(pool);
            }
            if (ZOMBIE_ENTITY.equals(id) || ZOMBIE_VILLAGER_ENTITY.equals(id) || HUSK_ENTITY.equals(id)) {
                LootPool.Builder pool = LootPool.builder()
                        ...
                        .with(ItemEntry.builder(VegetableContent.TOMATO));
                builder.pool(pool);
            }
        });
    }
