...
public class MarshmallowOnAStickItem extends Item {
	...
	public int getDefaultMaxBurnTime(Item item) {
		if(item == FoodBundle.MARSHMALLOW_ON_A_STICK) {
			return 150;
		}
		else if(item == FoodBundle.TOASTY_MARSHMALLOW_ON_A_STICK) {
			return 75;
		}
		else if(item == FoodBundle.GOLDEN_MARSHMALLOW_ON_A_STICK) {
			return 20;
		}
		else if(item == FoodBundle.BURNT_MARSHMALLOW_ON_A_STICK) {
			return 30;
		}
		else {
			return 60;
	...
	public Item getDefaultBurnItem(Item item) {
		if(item == FoodBundle.MARSHMALLOW_ON_A_STICK) {
			return FoodBundle.TOASTY_MARSHMALLOW_ON_A_STICK;
		}
		else if(item == FoodBundle.TOASTY_MARSHMALLOW_ON_A_STICK) {
			return FoodBundle.GOLDEN_MARSHMALLOW_ON_A_STICK;
		}
		else if(item == FoodBundle.GOLDEN_MARSHMALLOW_ON_A_STICK) {
			return FoodBundle.BURNT_MARSHMALLOW_ON_A_STICK;
		}
		else if(item == FoodBundle.BURNT_MARSHMALLOW_ON_A_STICK) {
			return Items.STICK;
		}
