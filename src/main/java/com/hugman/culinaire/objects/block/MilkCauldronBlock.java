...
public class MilkCauldronBlock extends BlockWithEntity {
	public static final IntProperty LEVEL = CulinaireBlocks.Properties.LEVEL_1_3;
	...
	public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
		...
		if(isReady) {
			if(!world.isClient) {
				...
				ItemEntity itemEntity = new ItemEntity(world, (double) pos.getX() + d, (double) pos.getY() + e, (double) pos.getZ() + g, new ItemStack(CulinaireItems.CHEESE, 2));
			...
			if(handStack.isEmpty()) {
				...
				if(item == Items.GLASS_BOTTLE) {
					if(i > 0 && !world.isClient) {
						if(!player.abilities.creativeMode) {
							ItemStack milkBottleStack = new ItemStack(CulinaireItems.MILK_BOTTLE);
							player.incrementStat(Stats.USE_CAULDRON);
				...
				else if(item == CulinaireItems.MILK_BOTTLE) {
