...
public class KettleScreen extends HandledScreen<KettleScreenHandler> {
    ...
    protected void drawBackground(MatrixStack matrices, float delta, int mouseX, int mouseY) {
        ...
        if (brewTime > 0) {
            ...
            if (brewBarHeight > 0) {
                drawTexture(matrices, i + 99, j + 17, 222, 0, 7, brewBarHeight);
            }
        }
        if (fluid == 0) {
            drawTexture(matrices, i + 65, j + 48, 176, 0, 46, 16);
        } else {
            if (fluidLevel > 0) {
                int teaColor;
                if (fluid == 2) {
                ...
                drawTexture(matrices, i + 65, j + 64 - fluidHeight, 176, 16, 46, fluidHeight);
            }
        }
        RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
        if (isHot) {
            drawTexture(matrices, i + 76, j + 68, 176, 32, 24, 9);
        }
    }
}
